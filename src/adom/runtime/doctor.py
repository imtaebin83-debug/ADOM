...
GPU_PROFILES: dict[str, dict[str, Any]] = {
    ...
    "rtx-pro-4500-blackwell-32gb": {
        "display_name": "NVIDIA RTX PRO 4500 Blackwell 32GB",
        "name_pattern": r"\bRTX PRO 4500 BLACKWELL\b",
        "memory_gib_range": (29.0, 34.0),
        "compute_capability": (12, 0),
        "proposed_micro_batches": [8, 4, 2, 1],
        "status": "proposal-until-memory-probe",
    },
    "rtx-5090-32gb": {
        ...
        "name_pattern": r"\b(?:GEFORCE )?RTX 5090\b",
        "memory_gib_range": (29.0, 34.0),
        "compute_capability": (12, 0),
        "proposed_micro_batches": [8, 4, 2, 1],
        "status": "proposal-until-memory-probe",
...
def validate_gpu_profile(
    ...
    compiled_arches: list[str] | None = None,
) -> dict[str, Any]:
    ...
    capability_matches = (
        ...
        else tuple(compute_capability) == expected_capability
    )
    native_arch = f"sm_{expected_capability[0]}{expected_capability[1]}"
    native_arch_supported = (
        None if compiled_arches is None else native_arch in compiled_arches
    )
    errors: list[str] = []
    warnings: list[str] = []
    if not name_matches:
    ...
    if capability_matches is False:
        errors.append(
            f"GPU profile {profile_name} requires compute capability "
            f"{expected_capability[0]}.{expected_capability[1]}; runtime reported "
            f"{compute_capability[0]}.{compute_capability[1]}"
        )
    if native_arch_supported is False:
        warnings.append(
            f"PyTorch was not compiled with native {native_arch} support; "
            "PTX JIT compatibility is provisional and the active B5 memory "
            "probe is mandatory before training"
        )
    return {
        ...
        "observed_compute_capability": (
            None if compute_capability is None else list(compute_capability)
        ),
        ...
        "compute_capability_matches": capability_matches,
        "native_arch_supported": native_arch_supported,
        "warnings": warnings,
        "errors": errors,
    }
...
def run_doctor(
    ...
    try:
        ...
        if gpu["available"]:
            ...
            gpu["compiled_arches"] = list(torch.cuda.get_arch_list())
    except Exception as error:  # pragma: no cover - exercised in container
    ...
    if gpu_profile is not None:
        gpu_profile_report = validate_gpu_profile(
            ...
            tuple(gpu["compute_capability"])
            if "compute_capability" in gpu
            else None,
            list(gpu["compiled_arches"]) if "compiled_arches" in gpu else None,
        )
        errors.extend(gpu_profile_report["errors"])
        warnings.extend(gpu_profile_report["warnings"])
