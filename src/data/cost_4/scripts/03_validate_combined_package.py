...
ALLOWED_IDS = {0, 1, 2, 3, 255}
SOURCES = {"rellis", "rugd", "ycor", "goose"}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate an ADOM Cost4 traversability package."
    )
...
def main() -> None:
    ...
    unknown_sources = sorted({row["source"] for row in rows} - SOURCES)
    if unknown_sources:
    ...
    for index, row in enumerate(rows, start=1):
        ...
        if invalid:
            raise ValueError(f"Invalid Cost4 IDs in {mask_path}: {invalid}")
        actual_ratio = float(np.count_nonzero(mask != 255) / mask.size)
        if abs(actual_ratio - float(row["non_ignore_ratio"])) > 1e-6:
            raise ValueError(f"non_ignore_ratio mismatch: {row['sample_key']}")
    ...
    report = {
        ...
        "class_pixel_counts": {str(key): class_pixels[key] for key in (0, 1, 2, 3, 255)},
        "class_frame_counts": {str(key): class_frames[key] for key in (0, 1, 2, 3, 255)},
    ...
    print(f"Cost4 validation PASS: {report_path}")
