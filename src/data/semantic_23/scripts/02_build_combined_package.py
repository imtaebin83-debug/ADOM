...
def resolve_relative(root: Path, value: str, field: str) -> Path:
    ...
    try:
        ...
        raise ValueError(f"{field} escapes source root: {value}") from exc
    return resolved
...
def read_manifest(source: str, root: Path) -> list[dict[str, str]]:
    ...
    required = {"sample_key", "source", "source_split", "sample_id", "image_path", "mask_path"}
    if not rows:
        raise ValueError(f"{source} bridge manifest is empty: {path}")
    if not required <= set(rows[0]):
        raise ValueError(f"{source} manifest requires {sorted(required)}")
    for row in rows:
        if row["source"] != source:
            raise ValueError(f"Source mismatch in {path}: {row['source']}")
    return rows
...
def main() -> None:
    ...
    for source in SOURCES:
        ...
        for row in read_manifest(source, root):
            ...
            if source == "rellis":
                if source_split not in main_splits:
                    raise ValueError(f"Invalid RELLIS split: {source_split}")
            ...
            combined_rows.append(
                {
                    ...
                    "non_ignore_ratio": row.get("non_ignore_ratio", ""),
                }
            )
